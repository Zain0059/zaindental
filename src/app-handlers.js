...
export async function doLogin() {
  ...
  try {
    ...
    if (err) err.textContent = "Connection error. Please check your network connection.";
    if (btn) { btn.textContent = "Sign In to Portal"; btn.disabled = false; }
  }
}
...
export function showApp() {
  ...
  if (hdrUser) hdrUser.textContent = currentUser?.full_name || "";
...
export function sw(tab) {
  ...
  if (titleEl) titleEl.textContent = titles[tab] || "Zain Dental Clinic";
...
export async function loadSched() {
  ...
  if (error) {
    el.innerHTML = '<div class="empty">Failed to load appointments</div>';
    return;
  }
  ...
  if (heroEl) {
    ...
    const dateFormatted = dObj.toLocaleDateString('en-US', options);

    heroEl.innerHTML = `
      <div class="today-hero-card">
        <div class="hero-top">
          <div>
            <div style="font-size:17px;font-weight:700;letter-spacing:-0.2px">Today's Clinic Overview</div>
            <div style="font-size:12px;color:#94A3B8;margin-top:2px">Real-time schedule &amp; patient flow</div>
          </div>
        ...
        <div class="hero-stats-grid">
          <div class="hero-stat-item">
            ...
            <div class="hero-stat-lbl">Appointments Today</div>
          </div>
          <div class="hero-stat-item">
            <div class="hero-stat-val" style="color:var(--teal-light)">${confirmedOrDone}</div>
            <div class="hero-stat-lbl">Confirmed / Done</div>
          </div>
          <div class="hero-stat-item" style="grid-column: span 2">
            <div style="font-size:11px;color:#94A3B8;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Next Upcoming Appointment</div>
            <div style="font-size:13px;font-weight:700;color:#FFFFFF;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
              ${nextAppt ? `⏱ ${(nextAppt.appointment_time || "").slice(0, 5)} · ${esc(nextAppt.patients?.first_name ? nextAppt.patients.first_name + ' ' + nextAppt.patients.last_name : nextAppt.patient_name || 'Patient')} (${esc(nextAppt.appt_type || 'General Treatment')})` : 'No upcoming appointments scheduled'}
            </div>
          </div>
        </div>
      </div>
  ...
  if (!visible.length) {
    el.innerHTML = '<div class="empty">No appointments scheduled for this day</div>';
    return;
  }
...
export function apptRow(a) {
  ...
  const pname = a.patients ? `${a.patients.first_name} ${a.patients.last_name}` : a.patient_name || "Unknown Patient";
  ...
  return `<div class="ar row-sep">
    ...
    <div class="ar-body" onclick='window.openAppt(${safeObj})'>
      ...
      <div class="ar-type">
        <span>🦷 ${esc(a.appt_type || "General Treatment")}</span>
        <span>·</span>
    ...
    <div class="ar-act">
      <select onchange="window.changeStatus(${a.id}, this.value)" onclick="event.stopPropagation()"
        ...
        <option value="completed" ${a.status === 'completed' ? 'selected' : ''}>● Done</option>
        <option value="no-show" ${a.status === 'no-show' ? 'selected' : ''}>● No-show</option>
        <option value="cancelled" ${a.status === 'cancelled' ? 'selected' : ''}>● Cancelled</option>
      </select>
      ${phone ? `<button class="wa-btn" onclick="event.stopPropagation();window.sendWA('${phone}','${waMsg}')" title="Send WhatsApp Reminder">
...
export async function openAppt(a) {
  document.getElementById("sh-appt-title").textContent = (a.appointment_time || "").slice(0, 5) + " · " + (a.appt_type || "General");
  ...
  document.getElementById("sh-appt-body").innerHTML = `
    <div style="background:var(--card);border-radius:var(--r-lg);border:1px solid var(--border);margin:16px;overflow:hidden;box-shadow:var(--shadow-md)">
      ...
      <div class="ig">
        <div class="ii"><div class="ii-lbl">Patient</div><div class="ii-val">${esc(pname)}</div></div>
        <div class="ii"><div class="ii-lbl">Date</div><div class="ii-val" style="font-family:var(--font-mono)">${dt}</div></div>
        <div class="ii"><div class="ii-lbl">Time</div><div class="ii-val" style="color:${c};font-size:18px;font-weight:800;font-family:var(--font-mono)">${t}</div></div>
        <div class="ii"><div class="ii-lbl">Procedure</div><div class="ii-val">${esc(a.appt_type || "General")}</div></div>
        <div class="ii"><div class="ii-lbl">Duration</div><div class="ii-val">${a.duration_minutes || 30} minutes</div></div>
        <div class="ii"><div class="ii-lbl">Dentist</div><div class="ii-val">${esc(a.dentist_name || "Dr. Abdullah Zain")}</div></div>
        ${a.notes ? `<div class="ii full"><div class="ii-lbl">Notes</div><div class="ii-val" style="font-size:13px;font-weight:400">${esc(a.notes)}</div></div>` : ""}
      </div>
    </div>
    <div class="pg" style="padding:0 16px 24px">
      <div class="slbl">Update Status</div>
      <div style="display:grid;grid-template-columns:repeat(3, 1fr);gap:8px;margin-bottom:20px">
        ${[["scheduled","#3B82F6","Scheduled"],["confirmed","#0EA5A4","Confirmed"],["completed","#10B981","Done"],["no-show","#64748B","No-show"],["cancelled","#EF4444","Cancelled"]].map(([s, col, lbl]) => `
      ...
      <div class="slbl">Patient WhatsApp Notification</div>
      <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:var(--r-lg);padding:16px;margin-bottom:16px">
        <div style="font-size:11px;font-weight:700;color:#15803D;margin-bottom:10px;display:flex;align-items:center;gap:6px;text-transform:uppercase;letter-spacing:.6px">
        ...
        <button onclick="window.open('https://wa.me/${intlPhone}?text=${waMsg}','_blank')"
          ...
          Send WhatsApp to ${esc(pname)} (${pat?.phone || ""})
        </button>` : `<div style="text-align:center;color:var(--error);font-size:12px;font-weight:600;padding:8px">No phone number on record for this patient</div>`}
      </div>
